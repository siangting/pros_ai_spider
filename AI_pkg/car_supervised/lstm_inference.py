...
class supervised_inference():
    ...
    def lstm_inference(self, node):
        while rclpy.ok():
            token = list()
            car_data = node.wait_for_data()
            token = self.get_wanted_features(car_data)
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def get_wanted_features(self, car_data):

        lidar_18 = []
        minimum = 999.0
        # print(len(car_data["lidar_data"]))
        for index in range(90):
            minimum = min(car_data["lidar_data"][index], minimum)
            if index % 5 == 0:
        ...
        token = str([car_data["car_target_distance"]] + [car_data["angle_diff"]] + lidar_18)
        return token
