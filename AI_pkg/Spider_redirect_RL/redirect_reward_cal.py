...
def reward_cal_main(data: dict, step_counter: int) -> float:
    ...
    distance_reward: float = (SpiderConfig.SPIDER_TARGET_INIT_DIST - current_spider_tree_dist) * redirect_PPOConfig.DISTANCE_REWARD_WEIGHT
    angle_penalty: float = (data["offset_angle"] - redirect_PPOConfig.REWARD_CAL_ANGLE_BASELINE) * redirect_PPOConfig.ANGLE_PENALTY_WEIGHT
